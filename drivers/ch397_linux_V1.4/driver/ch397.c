 ...
 *      - add support for mac address filtering
 */
...
#define VERSION_DESC  "V1.4 On 2024.07"
...
#define CH397_MCAST_FILTER_SIZE 8
...
struct ch397_mac_cfg {
	u8 multi_filter[CH397_MCAST_FILTER_SIZE];
};
...
static void ch397_set_multicast(struct net_device *net)
{
	...
	if (net->flags & IFF_PROMISC) {
		...
		val_set |= CH397_RX_CTRL_PBD | CH397_RX_CTRL_PUF | CH397_RX_CTRL_PAM;
	...
	memset(dp->mac_cfg.multi_filter, 0xFF, CH397_MCAST_FILTER_SIZE);
	if (net->flags & IFF_PROMISC) {
		...
		memset(dp->mac_cfg.multi_filter, 0, CH397_MCAST_FILTER_SIZE);
		netdev_for_each_mc_addr(ha, net) {
			crc_bits = ether_crc(ETH_ALEN, ha->addr) >> 26;
			dp->mac_cfg.multi_filter[crc_bits >> 3] |= 1 << (crc_bits & 7);
		}
	...
	set_bit(CH397_SET_RX_MF | CH397_SET_RX_MODE, &dp->flags);
...
static void work_func(struct work_struct *work)
{
	...
	__le32 value, hashval;
	int err;

	if (test_and_clear_bit(CH397_SET_RX_MF, &dp->flags)) {
		hashval = *(__le32 *)(dp->mac_cfg.multi_filter + 4);
		err = ch397_write(dp->dev, CH397_USB_WR_REG, CH397_ETH_MAC_HTHR, 4, &hashval);
		if (err < 0) {
			netdev_err(dp->dev->net, "%s, Error setting mac hash high reg.\n", __func__);
			return;
		}

		hashval = *(__le32 *)dp->mac_cfg.multi_filter;
		err = ch397_write(dp->dev, CH397_USB_WR_REG, CH397_ETH_MAC_HTLR, 4, &hashval);
		if (err < 0) {
